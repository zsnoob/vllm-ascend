...
查下来是权重内存布局的问题：vllm 的 `unquant_apply_mlp` 把权重按 `[E, 出, 入]` 存、
调用前 `transpose(1,2)`，而微基准原先直接按 `[E, 入, 出]` 连续存——
形状相同但布局不同，`npu_grouped_matmul` 走了慢路径。

| | 每块 FLOPs | 耗时 | 有效算力 |
|---|---|---|---|
| 真实 vLLM | 1.70 TFLOP | 7.9 ms | 215 TFLOPS（约 57% 峰值） |
| 微基准（修复前） | 1.70 TFLOP | 44 ms | 39 TFLOPS（约 10% 峰值） |

**已按 vllm 的布局修复，但尚未在硬件上验证修复是否生效。**
在验证之前，此前所有微基准结论中"通信被盖住 96~100%"这类数字都应视为
被虚高的专家计算撑出来的，需要重跑。
...
- [ ] 验证权重布局修复：专家计算应从约 44 ms 降到约 8 ms/块
- [ ] 用修复后的微基准重跑此前的 1A1M 与 V4 结论（覆盖率数字可能显著下降）
